...
export default function Navbar({ name, coinBalance, unreadCount, avatarUrl }: NavbarProps) {
  ...
  return (
    <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex items-center justify-end px-6 gap-3 shrink-0">
      <div
        className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-full px-3 py-1.5 text-sm font-semibold text-blue-700 dark:text-blue-300"
        aria-label={`${coinBalance.toLocaleString()} SkillCoins`}
        role="status"
      >
        <svg className="w-4 h-4 text-blue-500 dark:text-blue-400" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <a href="/profile" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
        ...
        <span className="text-sm font-medium text-gray-700 dark:text-gray-200 hidden sm:block">{name}</span>
      </a>
    </header>
  )
}
