...
export default function ExpertCard({ expert: e }: ExpertCardProps) {
  return (
    <Link href={`/expert/${e.id}`}>
      <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 hover:shadow-sm p-5 flex flex-col gap-4 h-full transition-all">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500">{e.field}</p>
          </div>
        </div>

        {e.bio && (
          <p className="text-xs text-gray-500 leading-relaxed line-clamp-2">{e.bio}</p>
        )}
